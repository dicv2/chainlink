...
func NewConfigPoller(lggr logger.Logger, lp LogPoller, addr common.Address, donID uint32, instanceType InstanceType, fromBlock uint64) LLOConfigPollerService {
	return newConfigPoller(lggr, lp, addr, donID, instanceType, fromBlock)
}

func newConfigPoller(lggr logger.Logger, lp LogPoller, addr common.Address, donID uint32, instanceType InstanceType, fromBlock uint64) *configPoller {
	cp := &configPoller{
		lp:           lp,
		addr:         addr,
		donID:        donID,
