...
func (s *shouldRetireCache) ShouldRetire(digest ocr2types.ConfigDigest) (bool, error) {
	...
	defer s.mu.RUnlock()
	_, exists := s.m[digest]
	s.eng.SugaredLogger.Debugw("ShouldRetire", "digest", digest, "exists", exists)
	return exists, nil
}
...
func (s *shouldRetireCache) checkShouldRetire(ctx context.Context) {
	...
	for _, log := range logs {
		...
		s.markRetired(log.BlockNumber, digest)
	}
}

func (s *shouldRetireCache) markRetired(latestBlockNum int64, configDigest ocrtypes.ConfigDigest) {
	s.mu.RLock()
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.latestBlockNum = latestBlockNum
