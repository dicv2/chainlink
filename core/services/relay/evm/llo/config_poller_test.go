...
func Test_ConfigPoller(t *testing.T) {
	...
	cpBlue := newConfigPoller(lggr, lp, addr, donID, InstanceTypeBlue, fromBlock, runReplay)
	cpGreen := newConfigPoller(lggr, lp, addr, donID, InstanceTypeGreen, fromBlock, runReplay)

	cfgCount := uint64(0)

	signers := []common.Address{common.Address{1}, common.Address{2}}
